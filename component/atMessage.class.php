...
* @ingroup 		phpAnvilTools
*/
class atMessage extends atControlAbstract
{
	const VERSION       = '1.1';
...
//                        FB::warn('Deleting message...');
					...
					$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'No Session Found - Unable to get message.', DevTrace::TYPE_WARNING);
				}
	...
	public function __set($propertyName, $value) {
		...
		switch ($propertyName) {
			case 'singleBox':
				if (!$this->singleBox) {
					$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Setting multiple box mode, changing separator...');
	...
	public function add($message) {
		if (isset($_SESSION)) {
			...
			$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Adding [' . $this->id . '] Message: ' . $message);
...
//            FB::log($_SESSION[$this->sessionName], '$_SESSION[$this->sessionName]');
			...
			$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'No Session Found - Unable to add message.', DevTrace::TYPE_WARNING);
		}
	}
	...
	public function delete() {
		if (isset($_SESSION)) {
			$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Deleting message...');
			unset($_SESSION[$this->sessionName]);
			return true;

		} else {
			$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'No Session Found - Unable to delete message.', DevTrace::TYPE_WARNING);
			return false;
		}
	}
...
//			}
			...
			if ($this->singleBox) {
				...
				for($i = 0; $i < sizeof($messages); ++$i) {
					$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Rendering message #' . $i . '...');
...
//        FB::log($return, 'atMessage->RenderContent()');
