 ...
 * @ingroup         phpAnvilTools atFuse
 */
class atFuseTrap extends atDynamicObjectAbstract
{
    const VERSION = '1.0';
    ...
    #----------------------------------------------
    #---- atFuseTrap Database Properties
    ...
    public function executeErrorHandler($type, $message, $file, $line)
    {
        ...
        //if (TRACE) atFuseTrap::addTraceInfo(__FILE__, __METHOD__, __LINE__, "parameters = ($errNo, $errFile, $errLine)");
        ...
        try {
            ...
            foreach ($this->_errorHandlerTypes as $key => $types) {
                //				$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, '$key=' . $key);
                //				$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, '$type=' . $type);
                //				$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, '$types=' . var_export($types, true));
                ...
                if (in_array($type, $types)) {
                    ...
                    $this->logGroup('atFuseTrap Error: ' . $subject,
                    ...
                    if (!empty($this->_errorHandlerCallback[$key])) {
                        //if (TRACE) atFuseTrap::addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Calling function ' . $this->_errorHandlerCallback[$key] . '...');
            ...
            echo('atFuseTrap ERROR: [' . $error->getCode() . '] ' . $error->getMessage() . ' on line ' . $error->getLine() . '<br>');
        }
    ...
    /**
     * atFuseTrap's exception handler.
     */
    public function executeExceptionHandler($exception)
    {
        ...
        try {
            ...
            //---- FireBug Console Output (via FirePHP)
            //            FB::group('atFuseTrap Exception: ' . $subject,
            //                array('Collapsed' => true,
            //                      'Color' => '#FF00FF'));

            $this->logGroup('atFuseTrap Exception: ' . $subject,
                array('Collapsed' => true,
                    'Color' => '#FF00FF'));
            ...
            echo('atFuseTrap ERROR: [' . $error2->getCode() . '] ' . $error2->getMessage() . ' on line ' . $error2->getLine() . '<br>');
        }
    ...
    public function onError($eventType = self::EVENT_TYPE_ERROR, $types = array(E_ERROR, E_CORE_ERROR, E_COMPILE_ERROR, E_USER_ERROR), $callback = '', $dieAfter = false)
    {
        //		$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Executing...');
        //        $this->logVerbose('Executing...');
        //if (TRACE) atFuseTrap::addTraceInfo(__FILE__, __METHOD__, __LINE__, "parameters = ($types, $callback, $dieAfter)");
    ...
    public function sendToServer($type, $subject, $number, $message, $file, $line)
    {
        ...
        if ($this->isTraceDefined()) {
            $trace = atFuseTrace::render();

            //			$this->_firePHP->log('Trace Defined');

            if ($this->echoTrace && $type > self::TRACE_TYPE_DEBUG) {
                echo(nl2br($message));
                echo(atFuseTrace::renderHTML());
                echo('<hr />');
            }
        }

        if ($this->isFireBugEnabled()) {
            //			$this->_firePHP->log(array($type, $subject, $number, $message, $file, $line, $trace));
        }

        if ($this->isServerEnabled()) {
            //			$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Opening Connection to atFuseTrap Server...');
            $this->logVerbose('Opening Connection to atFuseTrap Server...');

            try {
                $atFuseWS = new SoapClient($this->serverURL);

                //				$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Adding new event...');
                //                $this->logVerbose('Adding new event...');

                $return = $atFuseWS->newEvent(
                    $type,
                    $this->applicationID,
                ...
                $this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Server communications failure! (' . $fault->faultcode . ' - ' . $fault->faultstring . ')');
            } catch (Exception $exception) {
                $this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Server communications failure! (' . $exception->code . ' - ' . $exception->message . ')');
            }
            #----------------------------------------
        } elseif ($this->isDatabaseEnabled()) {
            //			$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Opening Connection to Database...');
            $this->logVerbose('Opening Connection to Database...');

            try {
                //				$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'starting...');
                //                $this->logVerbose('starting...');

                if ($this->tableVersion === 1) {
                    $objEvent = new atFuseEvent($this->dataConnection, $this->tableName);

                    $objEvent->eventTypeID = $type;
                    $objEvent->applicationID = $this->applicationID;

                } else {
                    $objEvent = new atFuseEvent2($this->dataConnection, $this->tableName);
                ...
                //				$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Save SQL = ' . $objEvent->buildSaveSQL());

                $objEvent->save();

                $return = array('result' => '1', 'junk' => 'junk');

                //				$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, '$return=' . var_export($return, true));

                return $return;
            } catch (Exception $exception) {
                //				$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Server communications failure! (' . $exception->getCode() . ' - ' . $exception->getMessage . ")\n[" . $exception->getTraceAsString() . ']');
                $this->logError('Server communications failure! (' . $exception->getCode() . ' - ' . $exception->getMessage() . ')');
            }

        } elseif (!empty($this->logPath)) {
            ...
            try {
                ...
                //				$this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Server communications failure! (' . $exception->code . ' - ' . $exception->message . ')');
                $this->logError('Server communications failure! (' . $exception->code . ' - ' . $exception->message . ')');
            }
    ...
    /**
     * Start all atFuseTrap processing.
     */
    ...
    /**
     * Start atFuseTrap's error handler.
     */
    ...
    /**
     * Start atFuseTrap's exception handler.
     */
    ...
    /**
     * Stop all atFuseTrap processing.
     */
    ...
    /**
     * Stops the atFuseTrap error handler and restores the prior error handler.
     */
