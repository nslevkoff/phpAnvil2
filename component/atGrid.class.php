<?php
/**
 * @see atControlAbstract
 * @see atCollection
 */
require_once('atControl.abstract.php');
require_once('atCollection.class.php');
//require_once('atForm.class.php');
//require_once('atComboBox.class.php');
require_once('atEntry.class.php');
//require_once('atButton.class.php');
 ...
 */
class atGrid extends atControlAbstract
{
...
//        $this->enableLog();
        ...
        $this->columns = new atGridColumns(true);

        $this->db = $atDataConnection;
        $this->baseSQL = $sql;
        $this->htmlID = $id;
...
//        $this->statePrefix = $this->htmlID . '_';
        ...
        $atGridCounter++;
        $this->statePrefix .= $atGridCounter . '_';

        if (is_object($atDataConnection)) {
            $this->_dataEngine = constant(get_class($atDataConnection) . '::ENGINE');
        }

        parent::__construct($id, $properties, $traceEnabled);

        $this->addTraceInfo(__FILE__, __METHOD__, __LINE__, '... done (' . date('h:i:s A') . ')');
    }
    ...
    public function renderHTML($atTemplate = null, $page = 1, $rows = 0)
    {
        return $this->render($atTemplate, $page, $rows);
    }
...
//        }
    ...
    public function render($atTemplate = null, $page = 1, $rows = 0)
    {
        //        global $firePHP;
        ...
        if (isset($this->atPageNav)) {
            $this->rowOffset = $this->atPageNav->itemOffset;
            $this->maxRows = $this->atPageNav->itemsPerPage;

            if ($this->pageNavHeaderEnabled || $this->pageNavFooterEnabled) {
                $pageNavHTML = $this->atPageNav->render();
            }

            //---- Render atPageNav Header -------------------------------------
        ...
        //---- Auto-Detect atGrid Variables ------------------------------------
        $this->addTraceInfo(__FILE__, __METHOD__, __LINE__, 'Auto-Detecting atGrid QueryString Variables...');
        ...
        //        $firePHP->log('$queryString = ' . $queryString);
        //        fb::Log($queryString, '$queryString');
        ...
        if ($objRS->read()) {
            ...
            $html .= '<table class="' . $this->mainClass . '">';
            ...
            if ($this->filterRowEnabled) {
                ...
                for ($objRS->columns->moveFirst(); $objRS->columns->hasMore(); $objRS->columns->moveNext())
                {
                    ...
                    if (!$columnOptions || $columnOptions->visible) {
                        ...
                        if (empty($entry)) {
                            $entry = new atEntry('', $this->statePrefix . $objColumn->name, $columnOptions->filterEntrySize, 40, '');
                            $html .= $entry->render();
                        }
                ...
                for ($objRS->columns->moveFirst(); $objRS->columns->hasMore(); $objRS->columns->moveNext()) {
                    ...
                    if (!$columnOptions || $columnOptions->visible) {
                        ...
                        if ($columnOptions && !empty($columnOptions->renderCallback)) {
                            ...
                            //                                $this->logDebug($columnOptions, '$columnOptions #1');
                            //
                            //                                $columnOptions = $this->columns->column($objColumn->name);
                            //
                            //                                $this->logDebug($columnOptions, '$columnOptions #2');
...
//                                            }
        ...
        //---- Render atPageNav Footer -----------------------------------------
        if (isset($this->atPageNav) && $this->pageNavFooterEnabled) {
            $html .= $pageNavHTML;
        }
    ...
    public function setColumnFormat($columnName, $formatType, $decimals = 0)
    {
        $this->_columnFormat[$columnName] = $formatType;
        $this->_columnFormatDecimals[$columnName] = $decimals;

        if ($formatType != self::COLUMN_FORMAT_NONE) {
...
//    }
    ...
    public function column($name, $addIfNotExist = false)
    {
        ...
        //        $firePHP->log($this->_columns);

        if (array_key_exists($name, $this->_columns)) {
            //            $firePHP->log('Column Found!');
            $return = $this->_columns[$name];
        } else {
            //            $firePHP->log('Column NOT Found!');
            if ($addIfNotExist) {
                $this->_columns[$name] = new atGridColumn();
                $return = $this->_columns[$name];
            } else {
                $return = false;
            }
    ...
    public function __get($propertyName)
    {
        ...
        //        $firePHP->log('Getting column (' . $propertyName . ')...');

        if (!array_key_exists($propertyName, $this->_columns)) {
            $this->_columns[$propertyName] = new atGridColumn();
    ...
    public function __isset($propertyName)
    {
        ...
        //        $firePHP->log('Is column (' . $propertyName . ') Set?');
    ...
    public function __set($propertyName, $value)
    {
        ...
        //        $firePHP->log('Setting column (' . $propertyName . ') to:');
        //        $firePHP->log($value);
    }
