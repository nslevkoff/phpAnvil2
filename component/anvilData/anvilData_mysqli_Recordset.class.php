...
* @ingroup 		phpAnvilTools anvilData anvilData_MySQL
*/
class anvilData_mysqli_Recordset extends anvilDynamicObjectAbstract
	implements anvilDataRecordsetInterface
{
	const VERSION	= '1.0';
	const ENGINE 	= 'mysqli';

	private $_anvilConnection;
	private $_columns;
	private $_row;
	...
	* @param string $result ?
	*/
	public function __construct($anvilConnection = null, $sql = null, $result = null)
	{
		$this->addProperty('result', '');
		$this->addProperty('rowNumber', 0);
		$this->addProperty('sql', '');

		$this->_anvilConnection = $anvilConnection;

		$this->result = $result;
		$this->sql = $sql;
		if (mysqli_errno($this->_anvilConnection->connection)) {
			$error_message = '<b>MySQL Error [' . mysqli_errno($this->_anvilConnection->connection) . '] ' . mysqli_error($this->_anvilConnection->connection) . "</b><br><br>\n";
